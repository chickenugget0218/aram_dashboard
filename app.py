...
탭 구성:
  📊 대시보드   : 업체별 카드(월별/일별) + 도넛(전체 비중)
  📁 파일 업로드: 월별 엑셀 / 일별 CSV 다중 업로드 → Supabase에 저장
  📋 미납현황   : 날짜별 미납 입력/저장 (RDB에 누적)
  💰 매출 이력 : monthly/daily 테이블을 표로 조회 (RDB 형태)

폴더 구조:
  <repo>/
    ├─ app.py
    ├─ requirements.txt
    ├─ .streamlit/secrets.toml      (로컬 전용, GitHub에 올리지 말 것)
    └─ logo/
        ├─ 엘지.png
        ├─ 비알.png
        └─ ...
...
# Storage 경로용 영문 코드 (한글 경로는 Supabase Storage에서 막힘)
COMPANY_CODES = {
    "엘지": "lg", "비알": "br", "에너자이저": "energizer", "라벨리": "labelly",
    "메디카": "medica", "남양": "namyang", "나사라": "nasara",
    "삼립": "samlip", "티젠": "tjeen",
}

# 업체별 합산에 포함할 제품코드 (이 매핑에 있는 업체만 필터링됨)
SELECTED_CODES = {
...
def parse_monthly(buf, filename, company=None):
    """월별 엑셀에서 (월 'YYYY-MM', 판매금액) 추출.
       company가 SELECTED_CODES에 있으면 그 코드들만 합산, 아니면 '금액 계' 사용."""
    ...
    # 3a) 선택 코드가 지정된 업체: 코드별 두 번째 줄(금액줄)의 판매금액만 합산
    ...
    if selected and sales_col is not None:
        ...
        # 3b) 그 외 업체: 기존 방식 — '금 액 계' 다음 행의 판매금액
        for i in range(nrows):
            v = df.iloc[i, 0]
...
def parse_daily(buf, filename, company=None):
    """일별 CSV에서 (날짜 'YYYY-MM-DD', 매출액) 추출.
       company가 SELECTED_CODES에 있으면 그 코드들만 합산."""
    ...
    for row in csv.reader(io.StringIO(text)):
        if len(row) < 7:
            continue
        # 선택 코드가 있으면 col 4(제품코드)가 그 안에 있는 행만
        if selected:
...
# ======================================================================
st.title("판매 대시보드")
st.caption("업체별 월별 판매금액 · 일별 누적 매출액 · 미납현황")
st.markdown(
    "<span style='font-size:12px; color:gray'>※ 일별 매출액은 그 달 1일부터 "
    "조회 전일까지의 누적입니다 (예: 6/17 조회 시 6/16까지 반영).</span>",
...
rank_of = {n: i + 1 for i, n in enumerate(COMPANIES)}


# ======================================================================
# 탭
# ======================================================================
tab_dash, tab_upload, tab_hist = st.tabs(
    ["📊 대시보드", "📁 파일 업로드", "💰 매출 이력"]
)
...
with tab_dash:
    ...
    with main_left:
        ...
        # ===== 일별 누적 매출액 순으로 카드 정렬 =====
        # 각 업체의 최신 일별 매출액(없으면 0)으로 순위 매기기
        ...
        # COMPANIES 순서를 latest_daily 큰 순으로 재정렬 (없는 업체는 0)
                    ...
                    with st.container(border=True):
                        ...
                        if ddf.empty:
                            ...
                            st.metric(
                                "일별 누적 매출액 (원)",
                                f"{latest_total:,}",
                                help=f"{latest_date} 기준 · 그 달 1일부터 "
                                     f"조회 전일까지 누적",
                            )

                        # ----- 아래: 월별 막대그래프 (금액 숫자 표시) -----
                        if sdf.empty:
                            st.caption("월별 데이터 없음")
                        else:
                            last = sdf.iloc[-1]
                            target = TARGETS.get(name, {}).get(last["월"])

                            st.caption("월별 판매금액 (원)")
                            sdf2 = sdf.assign(월라벨=sdf["월"].str[-2:])
                            base = alt.Chart(sdf2).encode(
                            ...
                            if target:
                                rule = (
                                    alt.Chart(pd.DataFrame({"목표": [target]}))
                                    .mark_rule(color="#1f77b4",
                                               strokeDash=[4, 4], strokeWidth=2)
                                    .encode(y="목표:Q",
                                            tooltip=[alt.Tooltip("목표:Q",
                                                                 format=",")])
                                )
                                layers.append(rule)
                            st.altair_chart(
    ...
    with main_right:
        st.subheader("전체 비중")
        # 양수인 업체만 도넛에 표시 (음수/0은 도넛 각도 계산에서 제외)
        ...
        if overall_d.empty:
            ...
            overall_d["라벨"] = overall_d.apply(
                lambda r: f"{r['업체']} {r['비중']:.1f}%", axis=1,
            )
        ...
        if st.button("💾 미납현황 저장", key="save_memo"):
            try:
                ...
                for _, r in edited.iterrows():
                    if not bool(r["미납여부"]):          # 체크 안 된 행은 저장 안 함
                        continue
                    payload.append({
                        "날짜": date_key,
                        "업체": r["업체명"],
    ...
    # ----- 목표 금액 설정 (LG 월별) -----
    st.divider()
    with st.expander("🎯 목표 금액 설정 (엘지 · 월별)"):
        company = "엘지"
        lg_df = monthly_df[monthly_df["업체"] == company]
        months = sorted(lg_df["월"].unique().tolist())
        if months:
            month = st.selectbox("월 선택", months)
        else:
            month = st.text_input("월 입력 (YYYY-MM)", value="2026-01")

        cur = TARGETS.get(company, {}).get(month, 0)
        new_target = st.number_input(
            f"{company} {month} 목표 금액 (원)",
        ...
        if st.button("저장", key="save_target"):
            sb.table("targets").upsert({
                "업체": company, "월": month, "목표금액": int(new_target),
            }, on_conflict="업체,월").execute()
            invalidate_cache()
            st.success(f"{company} {month} 목표를 {int(new_target):,} 원으로 저장했습니다.")
            st.rerun()
...
with tab_hist:
    st.subheader("매출 이력")
    st.caption("DB에 저장된 월별/일별 매출과 미납현황을 표로 조회합니다.")

    view = st.radio("조회 대상",
                    ["월별 매출 (monthly)", "일별 누적 매출 (daily)", "미납현황 (memos)"],
                    horizontal=True)

    c1, c2, c3 = st.columns(3)
    sel_company = c1.selectbox("업체", ["전체"] + COMPANIES, key="hist_company")
    ...
    # 필터
    if sel_company != "전체" and not df.empty:
    ...
    df = df.sort_values([date_col, "업체"]).reset_index(drop=True)
    if "id" in df.columns:
